...
# В этом файле можно создавать собственные словари с произвольными именами для последующего
# включения их внутрь 'oids' (имя зарезервировано).

# Словарь - это последовательность пар 'ключ':'значение'. В пользовательских словарях
# как имена ключей, так и их значения должны быть уникальными в пределах пространства имен
# соответствующей модели устройства.
# Например, для модели 'DES-3200-28' как ключ 'RX' так и значение '.1.3.6.1.2.1.2.2.1.10'
# могут использованы не более одного раза. При этом, конечно же, можно использовать ключи
# 'DS.1', 'DS.2' и т.д. с соответствующими уникальными значениями '{$oid}.1', {$oid.2}, etc.
...
CPUutil = {
    'CPU.' : '.1.3.6.1.4.1.171.12.1.1.6.2.0'
}
