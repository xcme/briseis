...
Сервис был разработан для получения большого количества метрик за небольшой интервал времени. При помощи **Briseis** можно обрабатыват большие объемы метрик за единицу времени. С учетом накладных операций (сбор, подготовка и отправка данных и статистики) сервис вполне справится с несколькими тысячами метрик в секунду.

Используя **Briseis** мы можно решать следующие задачи:
...
Помимо этого, каждая метрика имеет собственное *значение* и *временную метку*. На простом примере становится понятно, как вычисляются все упомянутые выше значения. Предположим, что у устройства с адресом '10.90.90.95' запрашивается  список ошибок на интерфейсах. Эта совокупность однородных метрик будет иметь одно общее имя, в нашем случае *RX_crc*. В конфигурационном модуля устройства это имя является ключом для опрашиваемого OID ('.1.3.6.1.2.1.16.1.1.1.8'). Упомянутый выше префик задается в свойствах каждого сервера Graphite. Пусть он будет, к примеру, равен 'sw.'. Все эти параметры известны еще до опроса оборудования, а после опроса становятся известны и остальные. Так, индексы интерфейсов в нашем  случае будут представлять собой *key*, а полученные значения -  *value*. Время, когда **получена** метрика - это *timestamp* (пусть оно будет равным '1451595600').
...
no_retries   | Список команд snmpset, для которых параметр *retries* всегда будет равен 0. Ресурсозатратные операции есть смысл помещать в этот список.
...
По умолчанию используется такой значение параметра *default_info*:
```
default_info = {
...
Параметр | Описание
-------- | --------
oids_set | Набор метрик, соответствующих OID, ключей и значений для **set**-операций. '**oids_set**' представляет собой словарь, где ключи соответствуют имени модели, а значения содержат обрабатываемые объекты
...
Параметр  | Описание
--------- | --------
oids_walk | Набор метрик и соответствующих OID для **walk**-операций. '**oids_walk**' представляет собой "список, где ключи соответствуют имени модели, а значения содержат соответствие имен метрики и опрашиваемых OID.
...
Способ опроса устройства зависит от того, как задан параметр *metric_name*. Если в имени хотя бы одной из метрик словаря присутствует **точка**, то все OID упаковываются в **get-request**. В противном случае OID упаковываются в **walk-request**. ***Это принципиальный для понимания логики работы программы момент***.
...
Для удобства использования параметры *oids_set* и *oids_walk* описаны в **bconfig.py**, но вложенные в них объекты размещаются в отдельных модулях. Каждой модели устойства соответствует одноименный модуль, расположенный в директории */devices*. При запуске **Brises** загружает эти модули и формирует в памяти полную структуру *oids_set* и *oids_walk*.
...
#### Добавлено
- Добавлена возможность отсылать метрики строго в одни и те же интервалы времени
- Добавлена возможность корректировать счетчики при обработке, что позволяет избежать всплесков на графиках
- Добавлена возможность выполнять walk-опросы раньше, чем set
- Добавлена возможность отправлять метрики на несколько серверов Graphite
- Добавлена возможность выполнять некоторые опросы без повторных попыток

#### Изменено
- Модели теперь определяются как в [swtoolz-core](https://github.com/xcme/swtoolz-core)
- Конфигурация теперь размещаются в отдельных модулях, как у [swtoolz-core](https://github.com/xcme/swtoolz-core)
...
#### Удалено
- Удалены отдельные настройки для Graphite
- Удалено взаимодействие с [Attractor](https://github.com/xcme/attractor), т.к. с точки зрения программы этот сервис теперь не отличается от Graphite.
- Из описания удалены некоторые примеры, которые показались избыточными и затрудняющими восприятие
- Файл userdict.py
